...
function FilmReel() {
  ...
  // Track scroll outside of useFrame to prevent forced synchronous layout (stuttering)
  ...
  useFrame((state) => {
    ...
    if (meshRef.current) {
      const scrollY = scrollRef.current; // Read from ref, NO layout thrashing
    ...
    <Float speed={2} rotationIntensity={0.5} floatIntensity={1}>
      <group ref={meshRef}>
        <mesh position={[0, 0.5, 0]} rotation={[Math.PI/2, 0, 0]}>
          ...
          <meshStandardMaterial color="#000000" metalness={0.9} roughness={0.1} transparent opacity={0.6} />
        </mesh>
        <mesh position={[0, -0.5, 0]} rotation={[Math.PI/2, 0, 0]}>
          <cylinderGeometry args={[3, 3, 0.1, 32]} />
          <meshStandardMaterial color="#000000" metalness={0.9} roughness={0.1} transparent opacity={0.6} />
    ...
    <group position={[-3, 0, -4]}>
        ...
        <group ref={directorRef} scale={0}>
          ...
          <mesh position={[-0.5, -1, 0]} rotation={[0, 0, 0]}>
             ...
             <meshStandardMaterial color="#333" metalness={0.5} roughness={0.2} transparent opacity={0.8} />
          </mesh>
        </group>
        ...
        <group ref={scoutRef} scale={0}>
           ...
           <mesh position={[-0.8, 0, 1.01]} rotation={[Math.PI/2, 0, 0]}>
             ...
             <meshStandardMaterial color="#000033" transparent opacity={0.8} roughness={0.1} metalness={0.9} emissive="#000033" emissiveIntensity={0.5} />
           </mesh>
           <mesh position={[0.8, 0, 1.01]} rotation={[Math.PI/2, 0, 0]}>
             <cylinderGeometry args={[0.55, 0.55, 0.05, 32]} />
             <meshStandardMaterial color="#000033" transparent opacity={0.8} roughness={0.1} metalness={0.9} emissive="#000033" emissiveIntensity={0.5} />
